...
def lcmv_beamformer(
    ...
    """
    Run full LCMV source estimation pipeline.
    """
    ...
    log.info(f"{'='*60}")
    log.info(f"LCMV Source Estimation: {subject_id} - {task}")
    log.info(f"{'='*60}")
    ...
    log.info(f"Clean-mode source estimation complete: {output_dir}")
    log.info(f"{'='*60}\n")
...
def execute_source_estimation(
    ...
    """
    High-level orchestrator for LCMV source estimation.
    
    Constructs standard paths within a BIDS-like project and delegates
    to lcmv_beamformer for the actual computation.
    """
    ...
    # Load data without logging (logging handled in lcmv_beamformer)
